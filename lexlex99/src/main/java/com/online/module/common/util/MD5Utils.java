...
public class MD5Utils {
    ...
    public static String b64_md5(String A) {
    	return binl2b64(core_md5(str2binl(A), A.length() * chrsz),A.length());
    }
    ...
    public static String binl2b64(int[] X,int length)
    {
        int[]   binarray = new int[(((X.length + 64) >>> 9) << 4) + 14 + 2];
    	for (int i = 0; i < X.length; i++) {
    		binarray[i] = X[i];
        }
      String tab = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
      String str = "";
      for(int i = 0; i < X.length * 4; i += 3)
      {
    	int a = (i   >> 2);
        int b = (i+1 >> 2);
        int c = (i+2 >> 2);
        
        int triplet = (((binarray[a] >> 8 * ( i   %4)) & 0xFF) << 16)
                    | (((binarray[b] >> 8 * ((i+1)%4)) & 0xFF) << 8 )
                    |  ((binarray[c] >> 8 * ((i+2)%4)) & 0xFF);
        for(int j = 0; j < 4; j++)
        {
